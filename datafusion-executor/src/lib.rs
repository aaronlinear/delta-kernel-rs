...
/// Lower a kernel declarative plan into a DataFusion logical plan without executing it.
///
/// Connectors that own an asynchronous DataFusion runtime can use this adapter, create the
/// physical plan with their session, and stream its partitions directly. [`DataFusionExecutor`]
/// remains available for kernel-internal synchronous [`PlanExecutor`] operations.
pub fn to_datafusion_plan(
    plan: &Plan,
...
/// executor:
///
/// - The session state holds DataFusion's optimizer rules, functions, catalogs, and runtime
///   environment whose object stores and resource limits are used by parquet and JSON scans. The
///   caller supplies it, directly or through a [`SessionContext`], so kernel plans use the
///   connector's DataFusion configuration. The executor clones it per query to create both the
///   physical plan and its matching task context.
...
impl DataFusionExecutor {
    ...
    pub fn new(
        ...
        Self::new_with_state(session_context.state(), storage_handler, runtime_handle)
    }

    /// Creates an executor from the exact DataFusion session state used by a query.
    ...
    fn execute_query(&self, plan: Plan) -> DeltaResult<PlanResult> {
        ...
        // DataFusion 55's leaf-expression pushdown can merge nested get_field expressions into a
        // projection while retaining the original struct alias, producing duplicate unqualified
        // fields (for example two `add` columns). Kernel scan plans already declare the exact
        // nested columns their sources read, so disable this optimization only for the cloned
        // state used to execute a kernel-owned plan.
        session_state
            .config_mut()
            .options_mut()
            .optimizer
...
mod tests {
    ...
    use delta_kernel::plans::ir::nodes::{
        FileType, Load, LoadColumnFileMeta, ScanFile, ScanParquet, Values,
    };
    ...
    fn constructor_does_not_modify_shared_session_state() {
        ...
        let executor_state = &executor.session_state;
    ...
    fn dynamic_scan_plan_with_schema(
        ...
        let dynamic_scan = Load::new(output_schema, file_type, file_meta, column_name!("dv"))
            .with_base_url(Url::parse("memory:///sidecars/").unwrap())

...
use delta_kernel::plans::ir::nodes::{FileType as KernelFileType, Load as KernelLoad};
...
/// Lowers a kernel [`Load`](KernelLoad) into a DataFusion
/// [`LogicalPlan::TableScan`](DFLogicalPlan::TableScan). `Load` is this fork's runtime-file scan
/// operator and corresponds to `DynamicScan` in the Delta Kernel plan stack.
///
...
pub(crate) fn lower_dynamic_scan(
    ...
    let _: KernelStructType = input.schema().as_arrow().try_into_kernel()?;
...
fn project_dynamic_scan_input(
    ...
    let column_count = 3 + scan.file_constant_columns.len();
    let mut columns = Vec::with_capacity(column_count);
    let path = column_to_df_expr(&scan.file_meta.path_column, schema)?;
    let path_index = push_aliased_column(&mut columns, path, "__dynamic_scan_path");
    let size = column_to_df_expr(&scan.file_meta.file_size_column, schema)?;
...
fn open_file(
    ...
    let resolved = match &scan.base_url {
        Some(base_url) => base_url.join(path).map_err(|error| {
            DataFusionError::Execution(format!(
                "dynamic scan could not resolve path `{path}` against `{base_url}`: {error}"
            ))
        })?,
        None => url::Url::parse(path).map_err(|error| {
            DataFusionError::Execution(format!(
                "dynamic scan path `{path}` is not an absolute URL: {error}"
            ))
        })?,
    };
...
mod tests {
    ...
    async fn input_projection_records_layout_and_preserves_dv_ancestor_nulls(
        ...
        let file_meta = delta_kernel::plans::ir::nodes::LoadColumnFileMeta::new(
            column_name!("file.path"),
            column_name!("file.size"),
            column_name!("file.filemod"),
        );
        let scan = KernelLoad::new(
            output_schema,
        ...
        .with_file_constant_columns(["partition", "version"]);
        ...
        assert_eq!(
            DFScalarValue::try_from_array(batch.column(3).as_ref(), 0)?,
